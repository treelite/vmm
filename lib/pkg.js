...
function importPackage(url, name = getName(url), force) {
...
export function init() {
    let items = get();
    if (!items.length) {
        return Promise.reject('nothing need to import');
    }

    return items.map(item => importPackage(item.url, item.name, true).then(() => item.name));
}
